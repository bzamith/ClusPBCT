...
public class LookAheadPBCT extends ClusInductionAlgorithm {
	...
	public LookAheadPBCT(ClusSchema schema, ClusSchema secondSchema, Settings sett) throws ClusException, IOException {
		super(schema, secondSchema, sett);
	}
	...
	public void initialize() throws ClusException, IOException {
		m_StatManager.initStatisticAndStatManager();
                m_SecondStatManager.initStatisticAndStatManager();
	}
		...
		if(type==FIRST_DATA){
                    ...
                    return node.getSecondFindBestTest().initSelectorAndStopCrit(node.getNodeVertical().getClusteringStat(), node.getSecondData());
                }
	}

	public ClusAttrType[] getDescriptiveAttributes(ClusNodePBCT node, int type) {
		ClusSchema schema = null;
                if(type == FIRST_DATA) schema = node.getSchema();
                else schema = node.getSecondSchema();
	...
	public void makeLeaf(ClusNode node) {
		...
		// Initialize selector and perform various stopping criteria
                ...
                initSelectorAndSplit(node, horizontal.getClusteringStat(),FIRST_DATA);
                initSelectorAndSplit(node, vertical.getClusteringStat(),SECOND_DATA);
                
		if (initSelectorAndStopCrit(node, FIRST_DATA)) {
			makeLeaf(horizontal);
                        makeLeaf(vertical);
			...
			return;
                ...
                if (initSelectorAndStopCrit(node, SECOND_DATA)) {
			makeLeaf(horizontal);
                        makeLeaf(vertical);
                        node.removeAllChildren();
			return;
...
//		long start_time = System.currentTimeMillis();
		...
		ClusAttrType[] attrs = getDescriptiveAttributes(node,FIRST_DATA);
		for (int i = 0; i < attrs.length; i++) {
			...
			if (at instanceof NominalAttrType) node.getFindBestTest().findNominal((NominalAttrType)at, node.getData());
                ...
                for (int i = 0; i < attrs.length; i++) {
                        ClusAttrType at = attrs[i];
                        node.getSecondFindBestTest().findNumeric((NumericAttrType)at, node.getSecondData());
                }

		// Partition data + recursive calls
                CurrentBestTestAndHeuristic best = node.getFindBestTest().getBestTest();
                
                double bestHorizontal = node.getFindBestTest().getBestTest().getHeuristicValue();
                double bestVertical = node.getSecondFindBestTest().getBestTest().getHeuristicValue();
                
                node.setTypeSplit(HORIZONTAL_SPLIT);
		if(bestVertical>bestHorizontal){
                    best = node.getSecondFindBestTest().getBestTest();
...
//			start_time = System.currentTimeMillis();
			if(node.getTypeSplit()==HORIZONTAL_SPLIT){
                            ...
                            for (int j = 0; j < arity; j++) {
                                    ...
                                    ClusSchema newSecondSchema = newSchema(node.getSecondSchema(),indexes[j]);
                                    RowData newSecondData = newData(node.getSecondData(),newSecondSchema,indexes[j]);
                                    ClusStatManager newSecondStatManager = new ClusStatManager(newSecondSchema,node.getSecondSchema().getSettings());
                                    newSecondStatManager.initStatisticAndStatManager();

                                    childVertical.initClusteringStat(newSecondStatManager, newSecondData);
                                    childVertical.initTargetStat(newSecondStatManager, newSecondData);

                                    initializeAttributeWeights(newSecondStatManager);

                                    ClusNodePBCT novo = new ClusNodePBCT(childHorizontal, childVertical, node.getStatManager(), newSecondStatManager, node.getSchema(), newSecondSchema, subsets[j], newSecondData);  
                                    node.setChild(novo,j);
                                    novo.infNode(HORIZONTAL_SPLIT, indexes[j]);
                                    novo.m_FTest = node.m_FTest;
                            ...
                            int[][] indexes = new int[arity][];
                            for (int j = 0; j < arity; j++) {
                                    subsets[j] = node.getSecondData().applyWeighted(test, j);
                                    indexes[j] = node.getSecondData().getIndexes(test,j);
                            }
                            ...
                            for (int j = 0; j < arity; j++) {
                                    ...
                                    ClusNode childVertical = new ClusNode();
                                    vertical.setChild(childVertical, j);
                                    childVertical.initClusteringStat(node.getSecondStatManager(), vertical.getClusteringStat(), subsets[j]);
                                    childVertical.initTargetStat(node.getSecondStatManager(), vertical.getTargetStat(), subsets[j]);
                                    ...
                                    ClusNodePBCT novo = new ClusNodePBCT(childHorizontal, childVertical, newStatManager, node.getSecondStatManager(), newSchema, node.getSecondSchema(), newData, subsets[j]);
                                    
                                    node.setChild(novo,j);
                                    novo.infNode(VERTICAL_SPLIT, indexes[j]);
                                    novo.m_FTest = node.m_FTest;
	...
	public void initSelectorAndSplit(ClusNodePBCT node, ClusStatistic stat, int type) throws ClusException {
            if(type==FIRST_DATA) node.getFindBestTest().initSelectorAndSplit(stat);
            else node.getSecondFindBestTest().initSelectorAndSplit(stat);
	}

	public void setInitialData(ClusNodePBCT node, ClusStatistic stat, RowData data, int type) throws ClusException {
            if(type==FIRST_DATA) node.getFindBestTest().setInitialData(stat,data);
            else node.getSecondFindBestTest().setInitialData(stat,data);
	}

	public void cleanSplit(ClusNodePBCT node, int type) {
                if(type==FIRST_DATA) node.getFindBestTest().cleanSplit();
                else node.getSecondFindBestTest().cleanSplit();
	}

        public ClusNodePBCT induceSingleUnpruned(RowData data, RowData secondData) throws ClusException, IOException {
                m_Root = null;
		...
		while (true) {
			...
			horizontal.getClusteringStat().showRootInfo();
                        ...
                        vertical.initClusteringStat(m_SecondStatManager, secondData);
			vertical.initTargetStat(m_SecondStatManager, secondData);
			vertical.getClusteringStat().showRootInfo();
                        
                        setNameAttributes();
                        m_StatManager.initClusteringWeights(getNameAttributes());
                        
                        m_Root = new ClusNodePBCT(horizontal, vertical, m_StatManager, m_SecondStatManager, m_Schema, m_SecondSchema, data, secondData);
                        
                        m_Root.initializeGlobalIndexes(secondData.getNbRows());
                        
                        initSelectorAndSplit(m_Root, horizontal.getClusteringStat(),FIRST_DATA);
			setInitialData(m_Root, horizontal.getClusteringStat(),data,FIRST_DATA);
                        
                        initSelectorAndSplit(m_Root, vertical.getClusteringStat(),SECOND_DATA);
			setInitialData(m_Root, vertical.getClusteringStat(),secondData,SECOND_DATA);
		...
		cleanSplit(m_Root,FIRST_DATA);
                cleanSplit(m_Root,SECOND_DATA);
		return m_Root;
	}
        

	public ClusModel induceSingleUnpruned(ClusRun cr) throws ClusException, IOException {
		return induceSingleUnpruned((RowData)cr.getTrainingSet(),(RowData)cr.getSecondTrainingSet());
	}
        ...
        public final double induceLookAhead(ClusNodePBCT node) throws ClusException, IOException{
            CurrentBestTestAndHeuristic best = node.getSecondFindBestTest().getBestTest();
            ClusNode vertical = node.getNodeVertical();
            ClusNode horizontal = node.getNodeHorizontal();
            vertical.testToNode(best);
            int arity = vertical.updateArity();
            ...
            for (int j = 0; j < arity; j++) {
                    ...
                    // FTest correction/adjustment
                    novo.m_FTest = (node.m_FTest*factor);
                    
                    heur[j] = induceHorizontalOneLevel(novo);
                    nodes[j] = novo;
            }
        ...
        public final double induceHorizontalOneLevel(ClusNodePBCT node) throws ClusException{
            initSelectorAndSplit(node, node.getNodeHorizontal().getClusteringStat(), FIRST_DATA);
