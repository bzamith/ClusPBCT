...
public class ClusNodePBCT implements ClusModel {
    ...
    protected FindBestTest m_FindSecondBestTest;
    ...
    public ClusNodePBCT(ClusNode horizontal, ClusNode vertical, ClusStatManager stat, ClusStatManager secondStat){
        m_Children.setSize(2);
        m_NodeVertical = vertical;
        m_NodeHorizontal = horizontal;
        m_StatManager = stat;
        m_SecondStatManager = secondStat;
        m_FindBestTest = new FindBestTest(m_StatManager);
        m_FindSecondBestTest = new FindBestTest(m_SecondStatManager);
    }
    
    public ClusNodePBCT(ClusNode horizontal, ClusNode vertical, ClusStatManager stat, ClusStatManager secondStat, ClusSchema schema, ClusSchema secondSchema){
        m_Children.setSize(2);
        m_NodeVertical = vertical;
        m_NodeHorizontal = horizontal;
        m_StatManager = stat;
    ...
    public ClusNodePBCT(ClusNode horizontal, ClusNode vertical, ClusStatManager stat, ClusStatManager secondStat, ClusSchema schema, ClusSchema secondSchema, RowData data, RowData secondData){
        m_Children.setSize(2);
        m_NodeVertical = vertical;
        m_NodeHorizontal = horizontal;
        m_StatManager = stat;
    ...
    public FindBestTest getSecondFindBestTest(){
        return m_FindSecondBestTest;
    }
    ...
    public void setSecondStatManager(ClusStatManager stat){
        m_SecondStatManager = stat;
    }
    ...
    public void setSecondSchema(ClusSchema schema){
        m_SecondSchema = schema;
    }
    ...
    public void setSecondData(RowData data){
        m_SecondData = data;
    }
    ...
    public ClusStatistic predictWeighted(DataTuple tuple, RowData secondData) {
        int nbTarget = tuple.getSchema().getNbTargetAttributes();
        int nbTree = secondData.getNbRows();
        ...
        for(int i=0; i<nbTree; i++){
            if(!res.getFilled(i)) { 
                ClusNodePBCT exit = predictWeightedNode(tuple, secondData.getTuple(i),i);
    ...
    public ClusNodePBCT cloneNode() throws ClusException, IOException{
        ...
        res.m_SecondData = (RowData) this.m_SecondData.cloneData();
        res.m_Schema = this.m_Schema;
        res.m_SecondSchema = this.m_SecondSchema;
        res.m_StatManager = this.m_StatManager;
        res.m_SecondStatManager = this.m_SecondStatManager;
	...
	res.m_StatManager.initHeuristic();
        ...
        res.m_SecondStatManager = new ClusStatManager(res.m_SecondSchema,res.getSecondSchema().getSettings());
	res.m_SecondStatManager.initHeuristic();
        res.m_SecondStatManager.initStopCriterion();
        res.m_SecondStatManager.initStatisticAndStatManager();*/
        res.m_FindBestTest = new FindBestTest(m_StatManager);
        res.m_FindSecondBestTest = new FindBestTest(m_SecondStatManager);
    ...
    public void correctHierarchy(RegressionStat pred){
        String hSeparator = this.m_Schema.getSettings().getHSeparator();
